...
class Train(object):
    ...
    def train(self):
        ...
        if self.config.Continue:
            ...
            if not self.config.checkpoint_path==None:
...
########################################################################
        ...
        for epoch in range(self.num_epochs):
            for batch_num, data in enumerate(self.train_loader):
                if True:
                    ...
                    if self.backbone == 'EGformer':
                        ...
                        depth,futures = self.depthnet(inputs)
            ...
            e_path = os.path.join(self.model_path, self.backbone + '-%d.pth' % (epoch))
    ...
    def sample(self,root,eval_name,crop_ratio):
        ...
        for image_path in eval_image:
            ...
            if self.backbone == 'EGformer':
                ...
                features,futures = self.depthnet(input_image)
                depth = features["pred_depth"]
            ...
            import cv2
            cv2.imwrite(os.path.join(self.eval_path,save_name ), depth)
